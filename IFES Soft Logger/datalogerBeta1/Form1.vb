...
Public Class Form1
    ...
    Private Sub btnConect_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnConect.Click
        ...
        Else
            MessageBox.Show("Selecciona un puerto de comunicación del Logger", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End If
        ...
        Else
            MessageBox.Show("Ritter no esta Conectado", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End If
    ...
    Private Sub SerialPort2_DataReceived(ByVal sender As Object, ByVal e As System.IO.Ports.SerialDataReceivedEventArgs) Handles SerialPort2.DataReceived
        Try
            If SerialPort2.IsOpen Then
                H = SerialPort2.ReadTo("H")
                II = SerialPort2.ReadTo("I")
                J = SerialPort2.ReadTo("J")
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message.ToString)
        End Try
    ...
    Private Sub promedios_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles promedios.Tick
        ...
        Dim cmd As String = "SELECT ensayo, AVG(velocidad) as vel,AVG(ch4) as ch4, AVG(co2) as co2,  AVG(h2s) as h2s, AVG(h2) as h2, AVG(temperatura) as promtemp1,AVG(templinea) as templinea, AVG(presionlinea) as presionlinea,AVG(tension) as tension, AVG(ph) as promph from data where ensayo = " & ensayoActual & " and reactor = '1' group by ensayo"
        Dim flujo1, flujo2, flujo3 As New DataTable

        'Diferencial de tiempos, este valor es numerico 
        Dim fechat As Date = Format(Now, "dd/MM/yyyy")
        tiempensayo = DateDiff(DateInterval.Day, fechaensayo, fechat)
        ...
        If promedios.Rows.Count > 0 Then
            ...
            'Totalizado de Produccion Diaria
            If tiempensayo = 0 Then
                txtPromgas.Text = H
            ElseIf tiempensayo > 0 Then
                txtPromgas.Text = H / (tiempensayo + 1)
                txtTotalGas.Text = H
            End If
    ...
    Private Sub txtflujo_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtflujo.TextChanged
